...
for band in range(bb,be+1):
    ...
    for sb in range(sbst,sbend):
        print 'Sub-band',sb
        sbs='%03i' % sb
        infile=troot+'_SB'+sbs+'_uv.killms.MS'
        outfile=troot+'_SB'+sbs+'_uv.killed.MS'
        if os.path.isdir(infile):
            ctod(infile,outfile)
            flist+='"'+outfile+'",'
    report('Concatenating band')
    outfile=troot+'_B'+bs+'_ckilled.MS'
    file=open('NDPPP-concat-killed.in','w')
    file.write('msin=['+flist[:-1]+']\nmsin.datacolumn = DATA\nmsout = '+outfile+'\nsteps = []\n')
    file.close()
    run('NDPPP NDPPP-concat-killed.in')
